...
class ProcessingThread(threading.Thread):
    ...
    def _create_depth_visualizations(self, depth_map_mm, target_mask=None):
        ...
        try:
            ...
            # Create masked depth visualization (target excluded)
            masked_depth_map = depth_map_mm.copy()
            if target_mask is not None and target_mask.shape == depth_map_mm.shape:
                # Set the area corresponding to the target mask to 0 (invalid)
                masked_depth_map[target_mask > 0] = 0 # Assuming mask is binary (0 or >0)
    ...
    def run(self):
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # <<< ADDED: Generate visualizations using the limited depth and target mask >>>
                depth_vis, masked_depth_vis = self._create_depth_visualizations(
                    depth_limited_proc_mm,
                    target_person_mask # Pass the mask obtained from YoloQR
                )
